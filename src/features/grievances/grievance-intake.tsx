...
export function GrievanceIntake({ firstName }: { firstName: string }) {
  ...
  return <main className={styles.page} id="main-content">
    <header className={styles.intro}><div><p className={styles.eyebrow}>Lodge Public Grievance</p><h1>Enter grievance details</h1><p>Welcome {firstName}. Describe the public service issue and the action requested.</p></div><aside><span aria-hidden="true">!</span><div><strong>Do not enter sensitive information</strong><small>Do not include Aadhaar bank details passwords OTPs or confidential documents.</small></div></aside></header>
    <div className={styles.journey}>
      ...
      <section className={styles.workspace}>
        {step === 1 ? <form onSubmit={analyse}>
          <div className={styles.sectionTitle}><span aria-hidden="true">01</span><div><h2>Grievance description</h2><p>Provide the service details relevant dates and the action requested.</p></div></div>
        ...
        {step === 2 && classification ? <div>
          <div className={styles.sectionTitle}><span aria-hidden="true">02</span><div><h2>{classification.needsHandoff ? "Use the designated service" : "Select concerned organisation"}</h2><p>Review the suggested organisation and change it if required.</p></div></div>
          {classification.needsHandoff ? <section className={styles.handoff}><span aria-hidden="true">↗</span><div><p className={styles.eyebrow}>{classification.category}</p><h3>{classification.department}</h3><p>{classification.reason}</p><a href={classification.needsHandoff.url} rel="noreferrer" target="_blank">{classification.needsHandoff.label} ↗</a></div></section> : <>
            <section className={styles.routeCard}><div className={styles.routeTop}><span className={styles.routeIcon} aria-hidden="true">◎</span><div><small>Suggested organisation</small><h3>{classification.department}</h3><p>{classification.category}</p></div><b>{classification.confidence}% match</b></div><div className={styles.reason}><strong>Basis of suggestion</strong><p>{classification.reason}</p></div></section>
      ...
      <aside className={styles.sideHelp}><p className={styles.eyebrow}>Instructions</p><h2>Include the following details</h2><ul><li><span>1</span>Name of the public service</li><li><span>2</span>Description of the issue</li><li><span>3</span>Relevant date or period</li><li><span>4</span>Action requested</li></ul><p>Review the grievance before submission. Do not include passwords OTPs or unnecessary personal information.</p><Link href="/help">View detailed instructions →</Link></aside>
    </div>
  </main>;
}
